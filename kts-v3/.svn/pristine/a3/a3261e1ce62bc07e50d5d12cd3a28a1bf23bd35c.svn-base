...
class Alipayment {
    ...
    function buildSignedParametersForApp($params) {
        ...
        return array('ordre'=>$paramStr,'order'=>$ordre,'sign'=>$this->signParameters($params));
    }
    ...
    public function alipay_refund($pay_sn,$order_amount){
        ...
        //require dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'config1.php';
        //商户订单号和支付宝交易号不能同时为空。 trade_no、  out_trade_no如果同时存在优先取trade_no
        //商户订单号，和支付宝交易号二选一
        //$out_trade_no = trim($_POST['WIDout_trade_no']);
    
        //支付宝交易号，和商户订单号二选一
        $trade_no = trim($pay_sn);
    
        //退款金额，不能大于订单总金额
        $refund_amount=trim($order_amount);
    
        //退款的原因说明
        $refund_reason=trim('正常退款');
