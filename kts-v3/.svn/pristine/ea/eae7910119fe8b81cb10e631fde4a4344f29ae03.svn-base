...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/Topic/tagList');?>";
    function search(){
...
<ol class="breadcrumb">
    ...
    <li>话题栏目设置</li>
    <li class="active"><a href="<?php echo U('Admin/Topic/tagList');?>">话题栏目类型管理</a></li>
</ol>
<ul id="myTab" class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">话题栏目管理</a></li>
    <li><a href="<?php echo U('Admin/Topic/tagAdd');?>">添加话题栏目类型</a></li>
</ul>
...
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($s["tag_id"]); ?></td>
                <td><?php echo ($s["tag_name"]); ?></td>
                <!--   <td style="width:50px;height:50px;"><img src="<?php echo ($s["imageurl"]); ?>" style="width:50px;height:50px;"></td> -->
                <!-- <td><?php echo ($s["imageurl"]); ?></td> -->
                <td><?php echo (mb_substr($s["tag_describe"],0,15,'utf-8')); ?></td>
                <td><?php echo ($s["post_num"]); ?></td>
                <td>
                    <div class="btn-group">
                        <a class="btn btn-warning" href="<?php echo U('Admin/Topic/tagEdit?tag_id='.$s['tag_id']);?>">修改</a>
                        
                         <?php if($s["is_show"] == 0): ?><a class="btn btn-danger disabled" href="#')">删除</a>
                        <?php else: ?>
                            <a class="btn btn-danger" href="javascript:showYesOrNo('确定要删除吗？','<?php echo U('Admin/Topic/delTag',array('tag_id'=>$s['tag_id']));?>')" >删除</a><?php endif; ?>
                      <!--   <a class="btn btn-danger" href="javascript:showYesOrNo('确定要删除吗？','<?php echo U('Admin/Topic/topicTagDel?tag_id='.$s['tag_id']);?>')">删除</a> -->
                    </div>
                </td>
