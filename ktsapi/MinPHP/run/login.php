...
<?php
    ...
    if($type  == 'do'){
        ...
        $sql = "select * from `admin` where login_name = '{$login_name}' and login_pwd = '{$login_pwd}' and isdel = '0'";
        $info = find($sql);
        if(!empty($info)){
            ...
            $time = time();
            $sql = "update admin set last_time = '{$time}' where id = {$info['id']}";
            update($sql);
            go(U());
        }else{
...
<div style="border:1px solid #ddd">
    <div style="background:#f5f5f5;padding:20px;position:relative">
        <h4>登录</h4>
        <div>
            <form action="?type=do&act=admin" method="post">
