...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/book/shareList');?>";
    function search(){
...
<ol class="breadcrumb">
    ...
    <li>分享书管理</li>
    <li class="active"><a href="<?php echo U('Admin/Book/bookList');?>"> 新书列表</a></li>
...
<div class="table-responsive" style="overflow: visible;">
    <table class="table table-striped table-hover">
        ...
        <?php if($list != false): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><a href="<?php echo U('Admin/Book/shareList?book_number='.$s['book_number']);?>"><?php echo ($s["book_number"]); ?></a></td>
                <td><img src="<?php echo ($s["cover_img"]); ?>"></td>
                <td><?php if(($s["status"]) == "1"): ?><a href="javascript:" title="点击禁用" onclick="changeState(this)" value="<?php echo ($s['status']>0?$s['status']:1); ?>" book_id="<?php echo ($s['book_id']); ?>"><span class="label label-success"><i class="glyphicon glyphicon-ok"></i></span></a>
                    <?php else: ?>
                    <a href="javascript:" title="点击启用" onclick="changeState(this)" value="<?php echo ($s['status']=0?$s['status']:0); ?>" book_id="<?php echo ($s['book_id']); ?>"><span class="label label-danger"><i class="glyphicon glyphicon-remove"></i></span></a><?php endif; ?>
                </td>
                <td><a href="<?php echo U('Admin/Book/bookDet',array('book_id'=>$s['book_id']));?>"><?php echo ($s["book_name"]); ?> / <?php echo ($s["author"]); ?></a></td>
                <td><a href="<?php echo U('Admin/Book/shareList',array('f_id'=>$s['fid']));?>"><?php echo ($s['category']); ?></a></td>
                <td><?php echo (date("Y-m-d",$s["share_time"])); ?></td>
                 <td><a href="<?php echo U('Admin/User/userDet?user_id='.$s['user_id']);?>"><?php echo ($s["user_name"]); ?></a></td>
                 
                <td><?php echo ($s['likes']?$s['likes']:0); ?></td>
                <td>
                    <div class="btn-group">
                        <a class="btn btn-info" href="javascript:" book_id="<?php echo ($s['book_id']); ?>" share_id="<?php echo ($s['share_id']); ?>">删除</a>                         
                    	<a class="btn btn-warning" href="<?php echo U('Admin/Book/bookEdit',array('book_id'=>$s['book_id'],'type'=>0));?>">编辑</a>
                    </div>
...
<script type="text/javascript">
	...
	function changeState(obj){
		...
		$.ajax({
			'type':"get",
			'datatyp':"json",
			'url':"<?php echo U('Admin/Book/changeState?book_id=');?>"+book_id,
			success:function(s){
				if(s){
					if(state==1){
						_this.html('');
   ...
   $('.btn-info').click(function(){
	   if(confirm("你确定要删除吗？")){
		   url="<?php echo U('Admin/Book/shareDel?book_id=');?>"+$(this).attr('book_id')+"&share_id="+$(this).attr('share_id');
		   $(this).attr('href',url)  
	   }else{
		   return false;
	   }  
