...
class OrderLogic extends Model
{
  ...
  private function MyOrderNo22(){
    ...
    {   
        ...
        $shop_name=$car_price['goods'][0]['shop_name'];
        ...
        if (empty($address))  return array('status'=>-9,'msg'=>'请选择正确的收货地址','result'=>'');       
       ...
       if(!$order_id)
        ...
        $b=logOrder($order_id,'您提交了订单，请等待系统确认','提交订单',$user_id);
        $c=self::addAddress($order_id, $address,$postscript);
    ...
    private function addOrderGoods($data,$order_id,$book_type,$shop_id){
        $sql ='insert kts_order_goods (order_id,book_id,type,book_name,book_desc,shop_id,shop_name,goods_number,book_thumb,book_number,publish_price,book_price,book_attr,goods_fee) values ';
        $book_id=null;
        foreach($data as $key => $val)
        {   
           
            $goods_fee=bcmul($val['goods_price'], $val['goods_number'],2);
            $shop_id=$val['shop_id']?$val['shop_id']:0;
            $shop_name=$val['shop_name']?$val['shop_name']:0;
            $book_attr=$val['book_attr']?$val['book_attr']:0;
            $book_desc=$val['book_desc']?$val['book_desc']:0;
            $sql .=' ('."{$order_id},"."{$val['book_id']},"."{$val['book_type']},"."'{$val['book_name']}',"."'{$book_desc}',";
            $sql .="$shop_id,"."'{$shop_name}',"."{$val['goods_number']},"."'{$val['cover_img']}',"."'{$val['book_number']}',";
            $sql .="{$val['market_price']},"."{$val['goods_price']},"."'{$book_attr}',"."{$goods_fee}),";       
        }
        $sql=substr($sql,0,-1);        
    ...
    * @return Ambigous <multitype:number string , multitype:number string unknown >|multitype:number string |\Think\mixed***/
    public function addBuyNowOrder($user_id,$book_id,$goods_number,$address_id,$coupon_id=0,$postscript=0){
        ...
        minus_stock_one($book_id,$goods_number);
    ...
    private  function findBook($user_id,$book_id,$goods_number,$coupon_id=0){
        ...
        if($book['type']==2){
            ...
            if($book['user_id']==$user_id) return array('error'=>1,'msg'=>'不能购买自己的书，请重新选择购买，谢谢!');
            ...
            if (check_myself($user_id, $book_id,2,$book['shop_id'])) return array('error'=>1,'msg'=>'不能购买自己的书，请重新选择购买，谢谢!');
    ...
    private function addOrdergoods2($order_id,$goods_number,$book){
        ...
        $publish_price=M('book_attr')->where(array('book_id'=>$book['book_id']))->getField('publish_price');
        $goods['publish_price']=$publish_price?$publish_price:0;
