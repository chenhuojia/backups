...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/BookComment/bDList');?>";
    function search(){
...
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($s["comment_id"]); ?></td>
                <td><a href="<?php echo U('Admin/Book/bookDet',array('book_id'=>$s['book_id']));?>"> <img src='<?php echo ($s["image"]); ?>' style="width:25px;height:25px;border-radius:25px;">  <?php echo ($s["book_number"]); ?> || <?php echo ($s["book_name"]); ?></a></td>
                <td><a href="<?php echo U('Admin/User/userList');?>&searchKey=<?php echo ($s["username"]); ?>"><img src='<?php echo ($s["imageurl"]); ?>'><?php echo ($s["username"]); ?></a></td>
                <td><?php echo ($s["content"]); ?></td>
                <td><?php echo (date("Y-m-d H:i:s",$s["comment_time"])); ?></td>
                <td><?php echo ($s["sums"]); ?></td>
                <td>
                    <div class="btn-group">
                        <a class="btn btn-success" href="<?php echo U('Admin/BookComment/book',array('book_id'=>$s['book_id']));?>">帖子详情</a>
                        <?php if($s["is_show"] == 0): ?><a class="btn btn-danger" href="<?php echo U('Admin/BookComment/changestatus?id='.$s['comment_id'].'&status=1');?>" title="点击启用">隐藏</a>
                        <?php else: ?>
                            <a class="btn btn-success" href="javascript:showYesOrNo('确定要隐藏吗？','<?php echo U('Admin/BookComment/changestatus?id='.$s['comment_id'].'&status=0');?>')" title="点击隐藏">显示</a><?php endif; ?>
					  <?php if($s["is_rec"] == 1): ?><a class="btn btn-danger" href="javascript:;"   val=0  onclick="rec(this,<?php echo ($s["comment_id"]); ?>)">取消推荐</a>
                      <?php else: ?> 
                       <a class="btn btn-success" href="javascript:;"   val=1  onclick="rec(this,<?php echo ($s["comment_id"]); ?>)">推荐</a><?php endif; ?>
                    </div>
                </td>
