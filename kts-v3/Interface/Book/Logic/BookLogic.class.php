...
class BookLogic  extends Model{
        ...
        if ($totype==0 && $type==2){                       
           ...
           $d=M('book_share')->where(array('book_id'=>$book_id))->setField('is_show',0);
            ...
            elseif ($type==2){
               ...
               $price=(isset($data['sell_price']) && $data['sell_price']>0)?$data['sell_price']:0;
               if ($price){$book['price']=$price;}
               $inventory=(isset($data['inventory']) && $data['inventory']>0)?$data['inventory']:0;
               if ($inventory){
                   $a=M('book_inventory')->where(array('book_id'=>$book_id))->setField('inventory',$data['inventory']);
               }else{
                   $a=1;
               } 
            }
    ...
    //添加视频数据
    public function addBookVideo($user_id,$book_id,$title='未知',$url,$img)
    {
        $arr = array(
              'book_id'=>$book_id,
              'user_id'=>$user_id,
