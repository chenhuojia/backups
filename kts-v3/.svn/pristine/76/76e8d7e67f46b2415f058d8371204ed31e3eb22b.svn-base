...
class IndexController extends AdminController {
	public function index() {
		...
		$user['IOS']=M('user')->alias('u')
		          ->join('left join kts_user_xq x on x.user_id = u.user_id')
		          ->where(array('u.is_show'=>0,'x.source'=>'IOS'))->count();
		$user['Android']=M('user')->alias('u')
		          ->join('left join kts_user_xq x on x.user_id = u.user_id')
		          ->where(array('u.is_show'=>0,'x.source'=>'Android'))->count();
		$this->user=$user;
		//$this->os = $os;
		...
		$user['create_time']=array(array('gt',$news),array('lt',$dead));
		$todo_info['user']=M('user_xq')->where($user)->count();		
        $share['share_time']=array(array('gt',$news),array('lt',$dead));
		$todo_info['share']=M('book_share')->where($share)->count();
		$book['time']=array(array('gt',$news),array('lt',$dead));
		$book['isdelete']=0;
		$todo_info['book']=M('book')->where($book)->count();
		$old_book['sell_time']=array(array('gt',$news),array('lt',$dead));
		$todo_info['old_book']=M('book_old')->where($old_book)->count(); 
