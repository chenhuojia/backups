...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/Auth/accessList');?>";
    function search(){
    ...
    $(function(){
        var sd = "<?php echo I('get.searchType','module');?>";
        $("#searchType").val(sd);
    })
</script>
<ol class="breadcrumb">
    ...
    <li>权限管理</li>
    <li class="active"><a href="<?php echo U('Admin/Auth/accessList');?>"> 权限列表</a></li>
</ol>
...
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($s["moduleName"]); ?></td>
                <td><?php if($s['status'] == 1): ?><span class="label label-success"><i class="glyphicon glyphicon-ok"></i></span>
                    <?php else: ?><span class="label label-danger"><i class="glyphicon glyphicon-remove"></i></span><?php endif; ?></td>
                <td>
                    <div class="btn-group">
                        <a class="btn btn-warning" href="<?php echo U('Admin/Auth/accessEdit?rule_id='.$s['rule_id']);?>">修改</a>
                        <a class="btn btn-danger" href="javascript:showYesOrNo('您确定删除吗？','<?php echo U('Admin/Auth/accessDel?rule_id='.$s['rule_id']);?>');">删除</a>
                    </div>
                </td>
