...
class Model {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                ...
                $fields =  F('_fields/'.base64_encode(strtolower($db.'.'.$this->tablePrefix.$this->name)));;
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
        ...
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            F('_fields/'.base64_encode(strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields));
        }
    }
   ...
   public function saveORadd($data,$key='编号'){
   	    if(empty($data[$key])){
     ...
     * @param string $parse
     */
    public function OneQuery($sql,$parse=false){
        if(!is_bool($parse) && !is_array($parse)) {
            $parse = func_get_args();
            array_shift($parse);
        }
