...
class AuthController extends AdminController {
    ...
    */
    public function groupDel($id){
        ...
        $where['id']=$id;
        //判断角色组下有没有用户
        $haveUser =$AGA->alias('ag')-> join('LEFT JOIN kts_auth_group_access as aga on ag.id = aga.group_id')->find($id);
        if ($haveUser['uid'])
        {
    ...
    */
    public function accessSelect(){
        ...
        $model=M('auth_rule');
        $mid=$model->alias('ar')->join('LEFT JOIN kts_auth_modules as mo on ar.mid = mo.id')
    ...
    */
    public function groupMember($n='15'){
        ...
        $model=M('system_user');
        $count=$model->alias('mb')->join('LEFT JOIN kts_auth_group_access as ag on mb.user_id = ag.uid')
            ->where($where)->count();
        $page=new \Think\Page($count, $n);
        $this->Page =$page->show();
        $result=$model->alias('mb')->join('LEFT JOIN kts_auth_group_access as ag on mb.user_id = ag.uid')
    ...
    */
    public function accessList($n=20){
        ...
        $model=M('auth_rule');
        $count=$model->alias('ar')->join('LEFT JOIN kts_auth_modules as mo on ar.mid = mo.id')
            ->where($where)->count();
        $Page = new \Think\Page($count, $n);
        $this->Page = $Page->show();
        $data=$model->alias('ar')->join('LEFT JOIN kts_auth_modules as mo on ar.mid = mo.id')
