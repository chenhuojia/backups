...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/Topic/topicList');?>";
    function search(){
    ...
    $(function(){
        var sd = "<?php echo I('get.groupType','all');?>";
        $("#groupType").val(sd);
...
<ul id="myTab" class="nav nav-tabs">
    ...
    <li><a href="<?php echo U('Admin/Topic/topicAdd');?>">添加帖子</a></li>
</ul>
            ...
            <label class="input-group-addon" for="groupType">帖子小组名：</label>
                <div class="input-group-btn">
                    <select name="groupType" id="groupType" class="form-control" data-live-search="false">
                        ...
                        <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["tag_id"]); ?>"><?php echo ($vo["tag_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
            </div>
        <div class="input-group">
...
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($i); ?></td>
                <td><?php echo ($s["tag_name"]); ?></td>
                <td><?php echo ($s["user_name"]); ?></td>
                <td><?php echo ($s["title"]); ?></td>
                <td><?php echo (date("Y-m-d H:i:s",$s["addtime"])); ?></td>
                <td><?php echo ($s["like_num"]); ?></td>
                <td><?php echo ($s["discuss_number"]); ?></td>
                <td>
                    <div class="btn-group">
                        <a class="btn btn-success" href="<?php echo U('Admin/SocialFeed/index?id='.$s['topic_id']);?>">帖子详情</a>
                        <a class="btn btn-warning" href="<?php echo U('Admin/Topic/topicEdit?topic_id='.$s['topic_id']);?>">修改</a>
                        <?php if($s["is_show"] == 0): ?><a class="btn btn-danger" href="<?php echo U('Admin/Topic/topicStatus?topic_id='.$s['topic_id'].'&tag_id='.$s['tag_id'].'&status=1');?>" title="点击启用">隐藏</a>
                        <?php else: ?>
                            <a class="btn btn-success" href="javascript:showYesOrNo('确定要隐藏吗？','<?php echo U('Admin/Topic/topicStatus?topic_id='.$s['topic_id'].'&tag_id='.$s['tag_id'].'&status=0');?>')" title="点击隐藏">显示</a><?php endif; ?>
					  <?php if($s["is_rec"] == 1): ?><a class="btn btn-danger" href="javascript:;"   val=0  onclick="rec(this,<?php echo ($s["topic_id"]); ?>)">取消推荐</a>
                      <?php else: ?> 
                       <a class="btn btn-success" href="javascript:;"   val=1  onclick="rec(this,<?php echo ($s["topic_id"]); ?>)">推荐</a><?php endif; ?>
                    </div>
                </td>
...
function rec(obj,id){
	...
	if(id>0){
		$.ajax({
			url:"<?php echo U('Admin/Topic/rec');?>",
			type:'post',
			data:'id='+id+'&val='+val,
			datatype:'json',
			success:function(e){
