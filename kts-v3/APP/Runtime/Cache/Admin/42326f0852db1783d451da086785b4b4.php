...
<script type="text/javascript">
    ...
    var search_url="<?php echo U('Admin/Information/regionList');?>";
    function search(){
...
<ol class="breadcrumb">
    ...
    <li>信息管理</li>
    <li class="active"><a href="<?php echo U('Admin/Information/regionList');?>">地域管理</a></li>
</ol>
<ul id="myTab" class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">地域管理</a></li>
    <li><a href="<?php echo U('Admin/Information/regionAdd');?>">添加地域</a></li>
</ul>
            ...
            <label class="input-group-addon" for="groupType">列表：</label>
                <div class="input-group-btn">
                    <select name="province_id" id="province_id" class="form-control" data-live-search="false">
                        ...
                        <?php if(is_array($provinceList)): $i = 0; $__LIST__ = $provinceList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["province_id"]); ?>"><?php echo ($vo["province"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
        </div>
        <div class="input-group">
    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr> 
                ...
                <td>
                    <div class="btn-group">
                        <a class="btn btn-warning" href="<?php echo U('Admin/Information/regionEdit?province_id='.$s['province_id'].'&city_id='.$s['city_id'].'&area_id='.$s['area_id']);?>">修改</a>
                    </div>
                </td>
...
<script type="text/javascript">
     $('#province_id').change(function(){
        ...
        $.ajax({
            cache:false,
            type:"POST",
            url:"<?php echo U('Admin/Information/CityInfo');?>",
            dataType:"json",
            data:objectModel,
     ...
     $('#city_id').change(function(){
        ...
        $.ajax({
            cache:false,
            type:"POST",
            url:"<?php echo U('Admin/Information/AreaInfo');?>",
            dataType:"json",
            data:objectModel,
            success:function(data){
                $("#area_id").empty();
