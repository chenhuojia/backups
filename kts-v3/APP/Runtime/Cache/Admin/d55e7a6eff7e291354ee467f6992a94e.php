...
<script type="text/javascript">
    var search_url="<?php echo U('Admin/User/userList');?>";
    function search(){
...
<ul id="myTab" class="nav nav-tabs">
    ...
    <li><a href="<?php echo U('Admin/User/userAdd');?>">用户注册</a></li>
</ul>
<div class="tab-content">
    ...
    <div class="form-inline" role="form">
        <div class="input-group">
            ...
            <input type="text" id="searchKey" name="searchKey" placeholder="" value="<?php echo ($_GET['searchKey']); ?>" class="form-control"/>
...
<div class="table-responsive" style="overflow: visible;">
    <table class="table table-striped table-hover">
        ...
        <?php if($list != false): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><a href="<?php echo U('Admin/User/userDet?user_id='.$s['user_id']);?>"><img src="<?php echo ($s["imageurl"]); ?>"></a></td>
                <td>
                    <a class="btn btn-default" title="点击浏览详细信息" href="javascript:void(0);" onclick="loadData('<?php echo ($s["name"]); ?>','<?php echo ($s["QQ"]); ?>','<?php echo (date("Y-m-d H:i:s",$s["create_time"])); ?>',
                    '<?php echo ($s["source"]); ?>','<?php echo ($s["sex"]); ?>','<?php echo ($s["city"]); ?>','<?php echo ($s["wechat"]); ?>','<?php echo ($s["phone"]); ?>','<?php echo ($s["introduce"]); ?>','<?php echo ($s["user_id"]); ?>',
                    '<?php echo ($s["last_location"]); ?>','<?php echo ($s["last_login_ip"]); ?>','<?php echo ($s["last_login_time"]); ?>')" role="button"><?php echo ($s["name"]); ?> / <?php echo ($s["phone"]); ?></a>
                </td>
                <td><?php if(($s["type"]) == "1"): ?><a href="javascript:" title="点击禁用" onclick="changeType(this)" value="<?php echo ($s["type"]); ?>" user_id="<?php echo ($s['user_id']); ?>"><span class="label label-success"><i class="glyphicon glyphicon-ok"></i></span></a>
                    <?php else: ?>
                    <a href="javascript:" title="点击启用" onclick="changeType(this)" value="<?php echo ($s["type"]); ?>" user_id="<?php echo ($s['user_id']); ?>"><span class="label label-danger"><i class="glyphicon glyphicon-remove"></i></span></a><?php endif; ?>
                </td>
                <td><a href="<?php echo U('Admin/Book/shareList');?>&user_id=<?php echo ($s["user_id"]); ?>"><?php echo ($s["share_count"]); ?></a></td>
                <?php if($s['shop_name'] == null): ?><td>暂时没有店铺</td>         
                <?php else: ?>
                 	<td><a href="<?php echo U('Admin/Shop/shopDet');?>&user_id=<?php echo ($s["user_id"]); ?>"><?php echo ($s["shop_name"]); ?></a></td><?php endif; ?>
                <td><a href="<?php echo U('Admin/User/userCollect?user_id='.$s['user_id']);?>"><?php echo ($s["collect_count"]); ?></a></td>
                 <td><a href="<?php echo U('Admin/Integral/integralDet?user_id='.$s['user_id']);?>"><?php echo ($s["integral"]); ?></a></td>
                 <td><a href="<?php echo U('Admin/Money/moneyDet?user_id='.$s['user_id']);?>"><?php echo ($s["money"]); ?></a></td>               
                 <td><a href="<?php echo U('Admin/Coupon/couponList?user_id='.$s['user_id']);?>"><?php echo ($s["coupon_count"]); ?></a></td>
                 <td><a href="<?php echo U('Admin/User/userCollect?user_id='.$s['user_id']);?>"><?php echo ($s["groups_count"]); ?></a></td>
                <td><?php echo ($s["last_login_time"]); ?></td>
                <td>
                    <div class="btn-group">

                       <!--  <a class="btn btn-primary" href="<?php echo U('Admin/User/userLearn?user_id='.$s['user_id']);?>">观看记录</a> -->
                        <a class="btn btn-info" href="javascript:" id="<?php echo ($s['user_id']); ?>">删除</a> 
                        <a class="btn btn-warning" href="<?php echo U('Admin/User/userEdit?user_id='.$s['user_id']);?>">编辑</a>
                        <div class="btn-group">
                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                更多 <span class="caret"></span>
                            </button>
                            <ul class="dropdown-menu">
                                <li><a href="<?php echo U('Admin/User/userChangepwd?user_id='.$s['user_id']);?>">修改密码</a></li>
                                <li><a href="javascript:showYesOrNo('重置后密码为123456，请及时修改。','<?php echo U('Admin/User/userResetpwd?user_id='.$s['user_id']);?>')">重置密码</a></li>
                            </ul>
                        </div>
                    </div>
                    <div id="class_name<?php echo ($s["user_id"]); ?>" style="display: none;">
...
<script type="text/javascript">
   ...
   $('.btn-info').click(function(){
	   if(confirm("你确定要删除吗？")){
		   url="<?php echo U('Admin/User/userDel?user_id=');?>"+$(this).attr('id');
		   $(this).attr('href',url)  
	   }else{
		   return false;
	   }	   	   
		...
		if(type==0){
			...
			if(data!='false'){
				$.ajax({
					'type':"post",
					'datatyp':"json",
					'url':"<?php echo U('Admin/User/changeType?');?>",
