...
class SystemController extends AdminController
{
    ...
    */
	public function userList($n = '15')
	{
		...
		$this->Page = $Page->show();
		$list = $admin->alias('us')->join('LEFT JOIN kts_auth_group_access as aga on aga.uid = us.user_id')
            ->join('LEFT JOIN kts_auth_group as ag on aga.group_id = ag.id')
    ...
    */
	public function userEdit($user_id = '0')
	{
        if (IS_POST) {
			...
			//获取帐号信息
			if ($user_id) {
				$info = $admin->alias('us')->join('LEFT JOIN kts_auth_group_access as aga on us.user_id = aga.uid')
...
//		}
		...
		//查询数据
		$model = M('feedback');
		$count = $model->alias('fb')->join('LEFT JOIN kts_user_owner as uo on fb.owner_id = uo.owner_id')
			->join('LEFT JOIN kts_store as s on s.store_id = fb.store_id')
			->where($where)->count();
		$Page = new \Think\Page($count, $n);
		$this->Page = $Page->show();
		$this->list = $model->alias('fb')->join('LEFT JOIN kts_user_owner as uo on fb.owner_id = uo.owner_id')
			->join('LEFT JOIN kts_store as s on s.store_id = fb.store_id')
    ...
    */
    public function InfoPushAll(){
		if(IS_POST) {
			...
			$result = $this->sendApp(I('post.content',''),'快图书',I('post.title',''));
			if($result !== false){
				$data['owner_id']=0;
				$data['type']='adv';
	...
	*/
    public function RedMsgList($n = '15')
    {
        ...
        //查询数据
        $model = M('msg');
        $count = $model->alias('m')->join('LEFT JOIN kts_user_owner as uo on m.owner_id = uo.owner_id')
            ->where($where)->count();
        $Page = new \Think\Page($count, $n);
        $this->Page = $Page->show();
        $this->list = $model->alias('m')->join('LEFT JOIN kts_user_owner as uo on m.owner_id = uo.owner_id')
    ...
    */
     ...
     * 设置列表
     */
    public function settingList($n = '15')
    {   
     ...
     * 编辑设置
     */
    public function settingEdit($config_id='0')
    {
        if (IS_POST)
        {
            $config = M('config');
            ...
            if($config_id !== false){
                ...
                $this->success('编辑成功', U('Admin/System/settingList'));
            }else{
    ...
    public function settingAdd()
    {
        if (IS_POST)
        {   
            $_POST['config_value_type'] = 1;
            $config = M('config');
            if (!$config->create($_POST,1)){
                // 如果创建失败 表示验证没有通过 输出错误提示信息
                $this->error($config->getError());
            }
            $lastId=$config->add($_POST);
            if ($lastId) {
                $this->addLog('config_name='.$_POST['config_name'].'&config_id='.$lastId,1);// 记录操作日志
                $this->success('新增设置成功', U('Admin/note/noteList'));
            }else{
                $this->addLog('config_name='.$_POST['config_name'].'&config_id='.$lastId,0);// 记录操作日志
                $this->error('新增设置失败');
            }
        }
        else
