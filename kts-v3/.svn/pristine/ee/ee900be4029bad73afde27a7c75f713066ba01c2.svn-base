...
<ol class="breadcrumb">
    ...
    <li>信息设置</li>
    <li class="active"><a href="<?php echo U('Admin/SMS/smsList');?>">banner管理</a></li>
</ol>
<ul id="myTab" class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">banner管理列表</a></li>
    <li><a href="<?php echo U('Admin/Information/bannerAdd');?>">添加banner管理</a></li>
</ul>
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <tbody> 
        <?php if(is_array($banners)): $i = 0; $__LIST__ = $banners;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr> 
                <td><?php echo ($i+$num); ?></td>
                <td><?php echo ($s["title"]); ?></td>
                <td ><img src="<?php echo ($s["imageurl"]); ?>" style="width:50px;height:50px;"></td>
                <!-- <td><?php echo ($s["url"]); ?></td> -->
                <td><?php if(($s["url"]) == ""): ?>没有填写
                    <?php else: ?>
                    <?php echo ($s["url"]); endif; ?>
                </td>
                <!-- <td><?php echo ($s["is_show"]); ?></td> -->
                <td><?php if(($s["is_show"]) == "0"): ?><a href="<?php echo U('Admin/Information/changeStatus?method=resume&id='.$s['id']);?>" title="点击启用"><span class="label label-danger"><i class="glyphicon glyphicon-remove"></i></span></a>
                    <?php else: ?>
                    <a href="<?php echo U('Admin/Information/changeStatus?method=forbid&id='.$s['id']);?>" title="点击禁用"><span class="label label-success"><i class="glyphicon glyphicon-ok"></i></span></a><?php endif; ?>
                </td>
                <td><?php if(($s["type"]) == "1"): ?>首页
                    <?php else: ?>
                    社区<?php endif; ?>
                </td>
                <!-- <td>
                   <?php if($s["type"] == 1): ?>首页
                    <?php elseif($s["type"] == 2): ?>社区
                    <?php else: ?> 消息回复<?php endif; ?>
                </td> -->
                <td>
                    <div class="btn-group">
                        <!-- <a class="btn btn-success" href="<?php echo U('Admin/SMS/smsDet?sms_id='.$s['sms_id']);?>">短信详情</a> -->
                        <a class="btn btn-warning" href="<?php echo U('Admin/Information/bannerEdit?banner_id='.$s['id']);?>">修改</a>
                    </div>
                </td>
